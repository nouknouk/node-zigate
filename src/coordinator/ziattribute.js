...
class ZiAttribute extends EventEmitter {
  ...
  setValue(newVal) {
    ...
    ZiAttribute.LOGS.debug(""+this.cluster.endpoint.device+""+this.cluster.endpoint+""+this.cluster+""+this+": value changed("+this.value+")");
		this.emit('attribute_change', newVal);
		this.cluster.emit('attribute_change', newVal);
		this.cluster.endpoint.emit('attribute_change', newVal);
		this.cluster.endpoint.device.emit('attribute_change', newVal);
		this.cluster.endpoint.device.coordinator.emit('attribute_change', newVal);
  }
}
